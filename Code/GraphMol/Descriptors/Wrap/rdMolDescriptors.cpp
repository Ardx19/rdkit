...
PyObject *runBatchToNumpy(python::list mols, Func descriptorFn) {
  auto batch = RDKit::Descriptors::extractMolPtrs(mols);
  std::vector<T> results = RDKit::Descriptors::runBatch<T>(batch.ptrs, descriptorFn);
  npy_intp dim = static_cast<npy_intp>(results.size());
  auto *arr = (PyArrayObject *)PyArray_SimpleNew(1, &dim, NpyType);
  memcpy(PyArray_DATA(arr), results.data(), results.size() * sizeof(T));
...
#ifdef RDK_BUILD_DESCRIPTORS3D
PyObject *CalcPBF_List(python::list mols, int confId) {
  return runBatchToNumpy<double>(mols, [confId](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::PBF(m, confId);
  });
}
PyObject *CalcPMI1_List(python::list mols, int confId, bool useAtomicMasses, bool force) {
  return runBatchToNumpy<double>(mols, [confId, useAtomicMasses, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::PMI1(m, confId, useAtomicMasses, force);
  });
}
PyObject *CalcPMI2_List(python::list mols, int confId, bool useAtomicMasses, bool force) {
  return runBatchToNumpy<double>(mols, [confId, useAtomicMasses, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::PMI2(m, confId, useAtomicMasses, force);
  });
}
PyObject *CalcPMI3_List(python::list mols, int confId, bool useAtomicMasses, bool force) {
  return runBatchToNumpy<double>(mols, [confId, useAtomicMasses, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::PMI3(m, confId, useAtomicMasses, force);
  });
}
PyObject *CalcRadiusOfGyration_List(python::list mols, int confId, bool useAtomicMasses, bool force) {
  return runBatchToNumpy<double>(mols, [confId, useAtomicMasses, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::radiusOfGyration(m, confId, useAtomicMasses, force);
  });
}
PyObject *CalcEccentricity_List(python::list mols, int confId, bool useAtomicMasses, bool force) {
  return runBatchToNumpy<double>(mols, [confId, useAtomicMasses, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::eccentricity(m, confId, useAtomicMasses, force);
  });
}
PyObject *CalcAsphericity_List(python::list mols, int confId, bool useAtomicMasses, bool force) {
  return runBatchToNumpy<double>(mols, [confId, useAtomicMasses, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::asphericity(m, confId, useAtomicMasses, force);
  });
}
PyObject *CalcSpherocityIndex_List(python::list mols, int confId, bool force) {
  return runBatchToNumpy<double>(mols, [confId, force](const RDKit::ROMol &m) {
    if (m.getNumConformers() == 0) return std::numeric_limits<double>::quiet_NaN();
    return RDKit::Descriptors::spherocityIndex(m, confId, force);
  });
}
#endif
...
BOOST_PYTHON_MODULE(rdMolDescriptors) {
 ...
 Default - Current RDKit default\n";
  ...
  python::def("CalcNumAromaticRings", CalcNumAromaticRings_List,
              (python::arg("mols")),
  ...
  python::def("CalcNumSaturatedRings", CalcNumSaturatedRings_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumHeterocycles", CalcNumHeterocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAromaticHeterocycles", CalcNumAromaticHeterocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAromaticCarbocycles", CalcNumAromaticCarbocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumSaturatedHeterocycles", CalcNumSaturatedHeterocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumSaturatedCarbocycles", CalcNumSaturatedCarbocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAliphaticRings", CalcNumAliphaticRings_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAliphaticHeterocycles", CalcNumAliphaticHeterocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAliphaticCarbocycles", CalcNumAliphaticCarbocycles_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAtoms", CalcNumAtoms_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumHeteroatoms", CalcNumHeteroatoms_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumAmideBonds", CalcNumAmideBonds_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumSpiroAtoms", CalcNumSpiroAtoms_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcNumBridgeheadAtoms", CalcNumBridgeheadAtoms_List,
              (python::arg("mols")),
              "returns the descriptor for a list of molecules as a numpy array (threaded); "
              "entries that are None or fail during calculation return NaN");
  ...
  python::def("CalcPBF", RDKit::Descriptors::PBF,
              (python::arg("mol"), python::arg("confId") = -1),
              docString.c_str());
  python::def("CalcPBF", CalcPBF_List,
              (python::arg("mols"), python::arg("confId") = -1),
              "Batch version of CalcPBF");
  ...
  python::def(
      ...
      docString.c_str());
  python::def(
      "CalcPMI1", CalcPMI1_List,
      (python::arg("mols"), python::arg("confId") = -1,
       python::arg("useAtomicMasses") = true, python::arg("force") = true),
      "Batch version of CalcPMI1");
  ...
  python::def(
      ...
      docString.c_str());
  python::def(
      "CalcPMI2", CalcPMI2_List,
      (python::arg("mols"), python::arg("confId") = -1,
       python::arg("useAtomicMasses") = true, python::arg("force") = true),
      "Batch version of CalcPMI2");
  ...
  python::def(
      ...
      docString.c_str());
  python::def(
      "CalcPMI3", CalcPMI3_List,
      (python::arg("mols"), python::arg("confId") = -1,
       python::arg("useAtomicMasses") = true, python::arg("force") = true),
      "Batch version of CalcPMI3");
  ...
  python::def(
      ...
      docString.c_str());
  python::def(
      "CalcRadiusOfGyration", CalcRadiusOfGyration_List,
      (python::arg("mols"), python::arg("confId") = -1,
       python::arg("useAtomicMasses") = true, python::arg("force") = true),
      "Batch version of CalcRadiusOfGyration");
  ...
  python::def(
      ...
      docString.c_str());
  python::def(
      "CalcEccentricity", CalcEccentricity_List,
      (python::arg("mols"), python::arg("confId") = -1,
       python::arg("useAtomicMasses") = true, python::arg("force") = true),
      "Batch version of CalcEccentricity");
  ...
  python::def(
      ...
      docString.c_str());
  python::def(
      "CalcAsphericity", CalcAsphericity_List,
      (python::arg("mols"), python::arg("confId") = -1,
       python::arg("useAtomicMasses") = true, python::arg("force") = true),
      "Batch version of CalcAsphericity");
  ...
  python::def("CalcSpherocityIndex", RDKit::Descriptors::spherocityIndex,
              ...
              docString.c_str());
  python::def("CalcSpherocityIndex", CalcSpherocityIndex_List,
              (python::arg("mols"), python::arg("confId") = -1,
               python::arg("force") = true),
              "Batch version of CalcSpherocityIndex");
